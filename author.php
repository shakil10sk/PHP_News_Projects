            ...
            <div class="col-md-8">
                ...
                <div class="post-container">
                    ...
                    <?php
                        ...
                        $auther_name_sql = "SELECT username FROM user WHERE user_id = {$authid}";
                        $auther_name_query = mysqli_query($conn,$auther_name_sql) or die("auther name fetch query error");
                        $auther_name_row = mysqli_fetch_assoc($auther_name_query);
                        echo "<h2 class='page-heading'>{$auther_name_row['username']}</h2>";
                    ...
                    <div class="post-content">
                        ...
                        <?php
                            ...
                            $sql_page = "SELECT * FROM post JOIn user ON user.user_id = post.author WHERE post.author = {$authid}";
                            ...
                            if ( $total_rows > 0) {
                                ...
                                if ($page > 1) {
                                    $prev_page = $page - 1;
                                    echo "<li><a href='author.php?auth_id={$authid}&page={$prev_page}'>Prev</a></li>";
                                }

                                for ($i = 1; $i <= $total_page; $i++) {
                                    $active = $i == $page ? "active" : "";
                                    echo "<li class='{$active}'><a href='author.php?auth_id={$authid}&page={$i}'>{$i}</a></li>";
                                }
                                if ($total_page > $page) {
                                    $next_page = $page + 1;

                                    echo "<li><a href='author.php?auth_id={$authid}&page={$next_page}'>Next</a></li>";
                                }
                                echo "</ul>";
                            }
                        ?>
