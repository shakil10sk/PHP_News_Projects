            ...
            <div class="col-md-8">
                ...
                <div class="post-container">
                  ...
                  <?php
                            ...
                            $category_name_sql = "SELECT category_name FROM category WHERE category_id = {$catid}";
                            $category_name_query = mysqli_query($conn,$category_name_sql) or die("category name fetch query error");
                            $category_name_row = mysqli_fetch_assoc($category_name_query);
                            echo "<h2 class='page-heading'>{$category_name_row['category_name']}</h2>";
                        ...
                        <?php
                            ...
                            $sql_page = "SELECT post FROM category WHERE category_id = {$catid}";

                            $page_result = mysqli_query($conn, $sql_page) or die("total Page count sql querry error");

                            $row_post = mysqli_fetch_assoc($page_result);
    
    
                            if ( mysqli_num_rows($page_result) > 0) {
                                $total_rows = $row_post['post'];
                                $total_page = ceil($total_rows / $limit);
    
                                echo "<ul class='pagination'>";
                                if ($page > 1) {
                                    $prev_page = $page - 1;
                                    echo "<li><a href='category.php?cat_id={$catid}&page={$prev_page}'>Prev</a></li>";
                                }
    
                                for ($i = 1; $i <= $total_page; $i++) {
                                    $active = $i == $page ? "active" : "";
                                    echo "<li class='{$active}'><a href='category.php?cat_id={$catid}&page={$i}'>{$i}</a></li>";
                                }
                                if ($total_page > $page) {
                                    $next_page = $page + 1;
    
                                    echo "<li><a href='category.php?cat_id={$catid}&page={$next_page}'>Next</a></li>";
                                }
                                echo "</ul>";
                            }
                            ?>
